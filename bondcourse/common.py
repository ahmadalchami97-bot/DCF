"""Shared context + drawing / explanation / chart helpers for the bond course."""
...
def clean_at(y):
    """Excel expression: clean price at yield `y` (coupon via PRICE, zero via manual PV)."""
    return f"IF(IsZero=1,Face/(1+({y}))^YrsToMat,PRICE(Settle,Mat,Cpn,{y},Face,Freq,Basis))"
...
def explain_box(sh, row, lines, *, last_col, title="What this sheet teaches"):
    return _box(sh, row, lines, last_col=last_col, title=title)


def interp(sh, row, lines, *, last_col, title="Explanation / Interpretation"):
