"""
Styling for the Portfolio Allocation Optimizer.

Implements the colour code as one ``apply()`` used by every sheet:
blue = input (on a pale-yellow highlight) · black = formula · green = key output /
cross-sheet link · navy/white = headers · amber panel = explanation boxes ·
red = warning. Input cells are left unlocked; formula/output cells are locked so
the user can opt into sheet protection without hiding their own inputs.
"""
...
_ROLES = {
    ...
    "colhdr_l": (_font(Palette.WHITE, 9, bold=True), _fill(Palette.SUBHEADER), A_L, B_NONE, True),
    ...
    "kpi": (_font(Palette.OUTPUT, 15, bold=True), _fill(Palette.PANEL), A_C, B_BOX, True),
    ...
    "good": (_font(Palette.GOOD, SZ, bold=True), None, A_C, B_NONE, True),
    "warn": (_font(Palette.WARN, SZ, bold=True), None, A_C, B_NONE, True),
    "bad": (_font(Palette.BAD, SZ, bold=True), None, A_C, B_NONE, True),
}
