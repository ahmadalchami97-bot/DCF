"""Sheet 13: Is This Bond Attractive? -- a simple, transparent /10 framework."""
...
def build(sh, ctx):
    common.title_block(sh, "IS THIS BOND ATTRACTIVE?", "A simple, transparent score out of 10 - and your own final view",
                       last_col=LAST)
    common.nav_bar(sh, 5)
    r = common.explain_box(sh, 8, [
        "This sheet gives the bond a score out of 10. It is a SIMPLE teaching framework, not a guarantee. It looks only "
        "at the yield, the duration (rate risk), the DV01 (money risk), where the bond sits on the curve, and whether "
        "it fits your rate outlook. It cannot know your personal situation - so the final view is editable.",
        "Higher score = the yield looks more worth the interest-rate risk. Read every row to see exactly how the points "
        "are earned - nothing is hidden.",
    ], last_col=LAST)
    r += 1
    ...
    rows = [
        ...
        ("Yield curve context", "=IF(N(Y10Y)=0,0,IF(Y10Y>Y2Y,1,0))", 1,
         "1 if the curve is normal / upward-sloping; 0 if flat, inverted, or no curve entered."),
        ("Rate outlook fit",
         '=IF(RateOutlook="Yields Rise",IF(ModDur<=4,1,0),IF(RateOutlook="Yields Fall",IF(ModDur>=7,1,0),'
         'IF(YTM>=BenchYield,1,0)))', 1,
         "Rise -> short duration scores; Fall -> long duration scores; Stable -> yield >= benchmark scores."),
    ]
    ...
    r = common.interp(sh, r, [
        ...
        "fit with your rate outlook. A higher total means the yield looks more worth the risk - but it is a starting "
    ...
    r += 1
    sh.put(r, 1, "Main reason it scores well", role="label_b")
    ...
    sh.put(r, 1, "Main risk to watch", role="label_b")
    ...
    r += 1
    sh.put(r, 1, "Final view (attractive?)", role="label_b")
    ...
    sh.put(r, 1, "In one sentence", role="label_b")
    ...
    sh.put(r, 1, "Your note (optional)", role="label_b")
    sh.put(r, 2, "", role="input_l"); sh.merge(r, 2, r, LAST); sh.row_height(r, 28); r += 2

    common.interp(sh, r, [
        "Reminder: this score is a simple, transparent framework, not a promise that the bond is a good investment. A "
        "bond can be right for one investor and wrong for another depending on horizon, cash needs and rate views. The "
        "final view above is editable - set it to whatever your own judgement says.",
