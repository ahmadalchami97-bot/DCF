"""Sheet 3: Cash Flows -- coupon & principal schedule, discounted at the yield."""
...
def build(sh, ctx):
    common.title_block(sh, "CASH FLOWS", "Every future payment, its timing, and its present value at the yield",
                       last_col=LAST)
    common.nav_bar(sh, 5)
    r = common.explain_box(sh, 7, [
        "WHAT THIS SHOWS:  each remaining coupon and the final principal, discounted back to today at the yield. "
        "The present values add up to the DIRTY price.",
        "Only payments AFTER settlement are shown. Notes/bonds pay 2 coupons per year; a bill pays only principal at maturity.",
        "Discount factor = 1 / (1 + yield/2) ^ (periods to payment).  PV = Total cash flow x Discount factor.",
    ], last_col=LAST)
    r += 1
    ...
    sh.put(r, 4, f"Future payments (capped at {MAX_CF_ROWS} rows).", role="note_l"); sh.merge(r, 4, r, LAST); r += 1
    ...
    cell_comment(sh, r, 2, "How much of the current coupon period is still ahead of you, so the first payment is "
                           "discounted over the correct partial period.")
    ...
    cell_comment(sh, hdr, 9, "Helper for convexity: Total x t x (t+1) / (1+y/2)^(t+2). Summed below.")
    cell_comment(sh, hdr, 10, "Helper for Macaulay duration: Time x PV. Summed and divided by total PV.")
    first = hdr + 1
    ...
    sh.put(r, 4, "Should equal the Dirty price on the Price & Yield sheet (checked in Audit).", role="note_l")
    ...
    cell_comment(sh, r, 2, "Weighted-average time to get your money back: sum(time x PV) / sum(PV).")
    r += 1
    ...
    sh.put(r, 4, "Excel has no convexity function, so it is computed from the schedule.", role="note_l")
    sh.merge(r, 4, r, LAST); r += 1
