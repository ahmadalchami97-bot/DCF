...
def build(sh, ctx):
    common.title_block(sh, "CASH FLOWS", "Every future payment, discounted back to today at the yield", last_col=LAST)
    common.nav_bar(sh, 5)
    r = common.explain_box(sh, 7, [
        "A bond's value is simply the present value (PV) of all its future cash flows - the coupons plus the "
        "principal repaid at maturity. Each payment is discounted back to today at the yield.",
        "For a zero-coupon bond there is only ONE cash flow: the principal at maturity.",
        "Only payments AFTER settlement are shown. The PVs add up to the DIRTY price - the engine behind the price.",
    ], last_col=LAST)
    r += 1
    ...
    cell_comment(sh, hdr, 9, "Helper for convexity: Total x t x (t+1) / (1+y/f)^(t+2). Summed below.")
    ...
    sh.put(r, 4, "Should equal the Dirty price on the Price & Yield sheet (checked in Quality Check).", role="note_l")
    sh.merge(r, 4, r, LAST); r += 1
