"""Sheet 7: Yield Curve / Benchmark Context."""
...
def build(sh, ctx):
    d = ctx.data["curve"]
    common.title_block(sh, "YIELD CURVE / BENCHMARK CONTEXT", "Where this bond sits on the government yield curve",
                       last_col=LAST)
    common.nav_bar(sh, 5)
    r = common.explain_box(sh, 7, [
        "The yield curve shows government bond yields across different maturities. It helps you see whether longer "
        "bonds pay more (normal) and where your bond sits versus other maturities.",
        "Enter the current government yields in the yellow cells; the chart and the read below update automatically.",
    ], last_col=LAST)
    r += 1
    ...
    r = common.interp(sh, r, [
        "NORMAL curve: long-term yields are higher than short-term yields - investors demand more yield to lend for "
        "longer. This is the usual, healthy shape.",
        "INVERTED curve: short-term yields are higher than long-term yields. It can happen when markets expect future "
        "rate cuts or slower growth, and is watched as a possible recession signal.",
        "FLAT curve: short and long yields are close - often a sign of uncertainty or a turning point in rate "
        "expectations.",
        "For this bond: a longer maturity sits further right on the curve and carries more interest-rate risk; a "
