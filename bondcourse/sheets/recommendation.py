"""Sheet 14: Recommendation Summary -- one page. Buy/Hold/Avoid stays manual."""
...
def build(sh, ctx):
    common.title_block(sh, "RECOMMENDATION SUMMARY", "One-page view of the bond and your final call", last_col=LAST)
    common.nav_bar(sh, 5)
    r = common.explain_box(sh, 8, [
        "This is the decision page. The workbook has done the maths; the Buy / Hold / Avoid call below is left as YOUR "
        "manual choice. The model informs the decision - it does not make it for you.",
    ], last_col=LAST)
    r += 1
    ...
    sh.put(r, 5, "From the Is This Bond Attractive? sheet.", role="note_l"); sh.merge(r, 5, r, LAST)
    r += 2
    ...
    left = [
        ...
        ("Maturity", "=Mat", Fmt.DATE), ("Coupon rate", "=Cpn", Fmt.PCT2),
    ...
    right = [
        ...
        ("Maturity bucket", '=IF(YrsToMat<=3,"Short-term",IF(YrsToMat<=10,"Intermediate-term","Long-term"))', None),
    ...
    common.interp(sh, r, [
        "BUY = the bond looks attractive on the score and your view. HOLD = acceptable, but not clearly attractive. "
